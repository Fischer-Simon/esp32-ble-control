 ...
 * Author: Jesse Laning
 *
 * 2024 Modified By: Simon Fischer
 * Make Arduino API compatible by replacing std::cout with configurable Stream.
 */
...
public:
    ...
    ArgumentParser(const std::string& bin, const std::string& desc, Print& out)
            : _bin(bin), _desc(desc), _stdout{out} {}
    ...
    void print_help(size_t count = 0, size_t page = 0) {
        ...
        for (size_t i = page * count;
            ...
            _stdout.printf("    %-23s%23s", name.c_str(), a._desc.c_str());
            if (a._required) {
                _stdout.print(" (Required)");
            }
            _stdout.println();
        }
    }
